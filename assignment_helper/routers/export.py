"""Stub router: export. Registered by app.py at the seam-freeze; owned by exactly one strand.
...
async def health() -> dict[str, object]:
    return {"router": "export", "implemented": IMPLEMENTED}


def not_built(what: str) -> HTTPException:
    return HTTPException(
        status_code=501,
        detail={"code": "export.not-built", "message": f"{what} is not built yet."},
    )
