"""Stub router: submit. Registered by app.py at the seam-freeze; owned by exactly one strand.
...
async def health() -> dict[str, object]:
    return {"router": "submit", "implemented": IMPLEMENTED}


def not_built(what: str) -> HTTPException:
    return HTTPException(
        status_code=501,
        detail={"code": "submit.not-built", "message": f"{what} is not built yet."},
    )
