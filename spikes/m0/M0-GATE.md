...
## 0. READ FIRST — this gate now runs against the shipped app

`spikes/m0/index.html` existed because there was no renderer yet. **There is now, and
it is strictly better**: real font metrics from a parsed font, KaTeX maths, procedural
paper, figures, and support for the user's own traced hand.

A gate should test **what ships**. The rendered half of this experiment is therefore
produced by the real application, not by the spike. The spike is kept for the record
and for its imperfection parameters, which the shipped paint engine inherited.

A prepared kit — five rendered sheets at 300 DPI from the real pipeline, the five
passages to copy by hand, and the full protocol — is generated by the lead on request.
The sections below remain the authoritative description of the experiment.
